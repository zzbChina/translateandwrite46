...
export default {
    name : 'UserCommentW',
    props:['user_writes','detailW','user','commentIndex'],
    data(){
        return{
            comment:{
                all:[],
    ...
    methods:{
        ...
        submitMyComment(){
            ...
            'target_id='+this.user_writes[this.commentIndex].id + '&' + 'target_title=' + this.detailW.w_title + '&'+ 'target_phone='+this.user_writes[this.commentIndex].user_phone+'&'
            +'target_nickname='+this.user_writes[this.commentIndex].user_nickname+'&'+'comment_time='+newTime + '&' + 'type=write'
    ...
    mounted(){
        //请求所有的该内容用户评论
        console.log(this.user_writes[this.commentIndex].id)
        this.$http.get(`/getAllUserComments?target_id=${this.user_writes[this.commentIndex].id}&type=write`).then(
            response=>{
                if(response.data != 'no'){
