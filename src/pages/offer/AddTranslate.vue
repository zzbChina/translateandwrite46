...
export default {
    ...
    methods:{
        ...
        submit(){ 
            if(this.title == '' || this.content == ''){
                ...
                this.isLoading = true
                let date = new Date()
                let message = 't_title=' + this.title + '&' + 't_level=' + this.level + '&' + 't_content=' + this.content + '&'
                + 't_author=' + this.user.user_nickname + 'author_phone=' + this.user.user_phone + '&' + 't_time=' + date + '&' + 't_check=' + 'wait'
                this.$http.post('/submitTranslate',message).then(
                    response=>{
    ...
    mounted(){
        ...
        let newDate = now.split('T')
        this.time = newDate[0].slice(1)
